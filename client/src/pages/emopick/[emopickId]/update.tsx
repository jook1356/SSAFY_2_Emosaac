...
import { returnEmopickDetailType } from "@/types/emopick";
import { putEmopick } from "@/api/emopick/putEmopick";
...
const Update = ({emopickData, emopickId}: {emopickData: returnEmopickDetailType; emopickId: number;}) => {
  ...
  const [title, setTitle] = useState(emopickData.title);
  ...
  function submitEmopick(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    if (title.length < 2 || title.length > 30) {
      ...
      setAddedBookList([]);
      setSelectedBookList([]);
      putEmopick({ emopickId, request, token }).then((res) => {
...
export const getServerSideProps = async (context: any) => {
  ...
  const data = await getEmopickDetail({ emopickId: params.emopickId, token })
    .then((res) => {
      return res;
    })
    .catch((err) => {
      console.log("pages/emopick/[emopickId]/update.tsx => ", err);
    });
