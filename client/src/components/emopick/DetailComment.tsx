...
import DetailCommentInput from "../DetailComment/DetailCommentInput";
import { returnCommentArrayType } from "@/types/comments";
import { getParentComments } from "@/api/comment/getParentComments";
import DetailCommentView from "../DetailComment/DetailCommentView";
...
const DetailComment = ({
  ...
  const criteriaList: { criteria: criteriaType; label: string }[] = [
    { criteria: "like", label: "인기순" },
    { criteria: "date", label: "최신순" },
  ];
  ...
  const criteriaRender = criteriaList.map((el, idx) => {
    return (
      <span
        key={`criteria-${idx}`}
        css={criteriaCSS({ elCriteria: el.criteria, curCriteria: criteria })}
        onClick={() => {
  ...
  return (
    <div ref={commentsWrapperRef} css={modalWrapperCSS({ isMobile })}>
      <div css={titleWrapperCSS}>댓글 ({totalCount})</div>
      <div css={criteriaWrapperCSS}>{criteriaRender}</div>
...
const criteriaCSS = ({
  ...
  return css`
    font-size: 14px;
    color: ${elCriteria === curCriteria
      ? "var(--back-color-4)"
      : "var(--text-color-4)"};
    margin-right: 8px;
    cursor: pointer;
    background-color: ${elCriteria === curCriteria
      ? "var(--text-color-4)"
      : "var(--back-color)"};

    box-shadow: ${elCriteria === curCriteria
      ? "none"
