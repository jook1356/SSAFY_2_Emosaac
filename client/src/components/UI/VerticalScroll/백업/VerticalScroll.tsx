...
const VerticalScroll = ({ API }: VerticalScrollProps) => {
  ...
  useEffect(() => {
    ...
    if (loadData) {
      ...
      API({ fetchedData: fetchedData, size: quantityPerPage }).then(
  ...
  useEffect(() => {
    if (getFetch === true && fetchedData) {
      ...
      API({
        ...
        prevId: lastContent.bookId,
        prevScore: lastContent.avgScore,
  ...
  const onScrollHandler = useMemo(
    () =>
      throttle((event) => {
        if (
          scrollWrapperRef?.current &&
          (scrollWrapperRef.current.scrollHeight - 10 <
        ...
        if (
          ...
          window.localStorage.setItem(
            "recent_scroll",
            String(scrollWrapperRef.current.scrollTop)
          );
        }
      }, 300),
    [fetchedData]
    ...
    return (
      ...
      >
        ...
        onScreenContentIdx === pageIdx + 1 ? (
          <div
            ref={actualPageRef}
            css={contentPageWrapperCSS({ isMobile, isTablet })}
          >
            {contentRender}
          </div>
        ) : (
