...
const SwipeableGallery = ({parentRef, content}: any) => {
  ...
  const onClickNextBtn = () => {
    if (movingDiv.current !== null && width !== null && contentCount < postData.content.length) {
  ...
  return (
    <div css={outerWrapperCSS}>
      <div css={prevBtnCSS} onClick={onClickPrevBtn}>〈</div>
      <div css={nextBtnCSS} onClick={onClickNextBtn}>〉</div>
      <Swipe onSwipeStart={(event:any) => {event.stopPropagation();}} onSwipeEnd={onSwipeEnd} onSwipeMove={onSwipeMove}>
        <div className={styles.wrapper}>
          
        
          <div className={styles.moveable} ref={movingDiv}>
            {postData.content.map((el:any) => {
                return <div className={styles.content} style={{width: width + 'px'}}>{el}</div> ; //, height: height + 'px'
            })}
          </div>
