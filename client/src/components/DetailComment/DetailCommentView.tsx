...
const DetailCommentView = ({
  ...
  const refreshCommentsHandler = () => {
    if (position === 0) {
      getParentComments({ bookId, criteria }).then(
      ...
      getChildComments({ parentId, criteria: "date" }).then(
        (res: returnCommentArrayType | null) => {
          if (res !== null) {
            setComments(() => res);
            setOffset(() => 2);
  ...
  const getCommentsHandler = () => {
    if (position === 0) {
      ...
      getChildComments({ parentId, criteria: "date", offset }).then(
        (res: returnCommentArrayType | null) => {
          if (res !== null) {
            setComments((prev) => [...prev, ...res]);
            setOffset((prev) => prev + 1);
      ...
      throttle((event) => {
        if (position === 0) {
          if (
            ...
            (commentsWrapperRef.current.scrollHeight - 10 <
              commentsWrapperRef.current.clientHeight ||
  ...
  const commentsRender = comments.map((el, idx) => {
    return (
      <DetailCommentViewElement
        key={`${idx}${el.commentId}${el.likeStatusSize}`}
        bookId={bookId}
        comment={el}
  ...
  const noCommentsRender = (
    <div css={noCommentsWrapperCSS}>댓글을 작성해 주세요!</div>
  );
  ...
  return (
    ...
    >
      ...
      {position === 1 && (
        <div css={childCommentsInputWrapperCSS}>{inputRender}</div>
      )}
      {position === 1 &&
        comments.length !== 0 &&
        comments.length < comments[0]?.totalCount &&
